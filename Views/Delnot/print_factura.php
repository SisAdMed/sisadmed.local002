<?php
    ...
    class PDF extends FPDF{
        ...
        $this->Cell(60);
        $this->SetFont('Arial','B',7);
        $this->Cell(25, 4,html_entity_decode($this->nom_tdoc) .': ' . $this->num_tdo . ' Tasa de cambio Bs.: ' . number_format($this->tasa_cambio, 4, ",", "."), 0, 1);
...
for ($i = 0; $i < count($r); $i++) {
    ...
    if($r[$i]->codigo_moneda != $r[$i]->moneda_emp){
        ...
        $pdf->MultiCell(90, 3, html_entity_decode($cadena), 0, 'L');
        $pdf->ln(-3);
        $pdf->Cell(90);
        if($r[$i]->iva_prod == 'S'){
