...
class Downloader
{
    ...
    {
        //Variables
        $filename = "Cotizacion " . $r[0]->num_tdo . '-' . $r[0]->nom_ent;
        $ext = ".xlsx";
        //Pprocedimiento
        ...
        //Cliente
        $sheet->setCellValue("A6", 'Nombre:')->getStyle("A6")->getFont()->setSize(8);
        $sheet->mergeCells('B6:E6')->setCellValue('B6', $r[0]->nom_ent)
            ->getStyle('B6:E6')->getFont()->setBold(true)->setSize(8);
        if(strlen($r[0]->nom_ent) > 44){
            $sheet->getStyle('B6:E6')->getAlignment()->setWrapText(true);
            $sheet->getRowDimension(6)->setRowHeight(30); // Ejemplo con altura fija
        }
        //Fecha
        $sheet->mergeCells('F6:G6')->setCellValue('F6', "Fecha: " . formatFecha($r[0]->fecha_comp))
            ->getStyle('F6:G6')->getFont()->setSize(8);
        $sheet->getStyle('F6:G6')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER_CONTINUOUS);
        //Moneda
        $sheet->mergeCells('H6:I6')->setCellValue('H6', "Moneda: " . $r[0]->codigo_moneda)
            ->getStyle('H6:I6')->getFont()->setSize(8);
        $sheet->getStyle('H6:I6')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER_CONTINUOUS);
        //RIF
        $sheet->setCellValue("A7", 'RIF:')->getStyle("A7")->getFont()->setSize(8);
        $sheet->mergeCells('B7:E7')->setCellValue('B7', $r[0]->rif_ent)
            ->getStyle('B7:E7')->getFont()->setBold(true)->setSize(8);
        //Cotización
        $sheet->mergeCells('F7:G7')->setCellValue('F7', $r[0]->nom_tdoc .": " . $r[0]->num_tdo)
            ->getStyle('F7:G7')->getFont()->setBold(true)->setSize(10);
        //Tasa
        $sheet->mergeCells('H7:I7')->setCellValue('H7', 'Tasa de Cambio Bs.: ' . number_format($r[0]->tasa_cambio, 2, ",", "."))
            ->getStyle('H7:I7')->getFont()->setBold(true)->setSize(8);
        //Dirección
        $sheet->setCellValue("A8", 'Dirección')->getStyle("A8")->getFont()->setSize(8);
        $sheet->mergeCells('B8:I8')->setCellValue('B8', htmlentities($r[0]->dir_ent))
            ->getStyle('B8:I8')->getFont()->setBold(true)->setSize(8);
        // Establecer el ajuste de texto para la celda C3
        $sheet->getStyle('B8:I8')->getAlignment()->setWrapText(true);
        $sheet->getRowDimension(8)->setRowHeight(30); // Ejemplo con altura fija
        ...
        $sheet->setCellValue("H10", 'IVA')->getStyle('H10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->setCellValue("I10", 'CANT.')->getStyle('I10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->setCellValue("J10", 'PRECIO')->getStyle('J10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        $sheet->setCellValue("K10", 'TOTAL USD')->getStyle('K10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);;
        $sheet->setCellValue("L10", 'TOTAL VES')->getStyle('L10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);;

        $sheet->getStyle('A10:L10')->getFont()->setBold(true)->setSize(10);
        ...
        for ($i = 0; $i < $totrows; $i++) {
            ...
            $total += $sub_total;
            if($r[$i]->iva_prod == 'N'){
                $sheet->setCellValue("H$line_item", "(E)")->getStyle("H$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sub_exe += $sub_total;
            }else{
                $sheet->setCellValue("H$line_item", number_format($GLOBALS['tasa_iva'], 2, ",", "."))->getStyle("H$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet->getCell("H$line_item")
                    ->getStyle()->getNumberFormat()
                    ->setFormatCode('#,###.00');
                    $x_iva = $GLOBALS['tasa_iva'];
                    $sub_imp += $sub_total;
                    $sub_iva +=  ($sub_total * ( $x_iva / 100 ));
                    $total += ($sub_total * ($x_iva / 100));
            }
            $sheet->setCellValue("I$line_item", $r[$i]->can_det)->getStyle("I$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            $sheet->getCell("I$line_item")
                ->getStyle()->getNumberFormat()
                ->setFormatCode('#####');
            $sheet->setCellValue("J$line_item", $r[$i]->pre_vta)->getStyle("J$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            $sheet->getCell("J$line_item")
                ->getStyle()->getNumberFormat()
                ->setFormatCode('#,###.00');
            $sheet->setCellValue("K$line_item", $r[$i]->sub_total)->getStyle("K$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            $sheet->getCell("K$line_item")
                ->getStyle()->getNumberFormat()
                ->setFormatCode('#,###.00');
            $sheet->setCellValue("L$line_item", $sub_total * $r[0]->tasa_cambio)->getStyle("L$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            $sheet->getCell("L$line_item")
                ->getStyle()->getNumberFormat()
                ->setFormatCode('#,###.00');
            $line_item++;
        }
        $sheet->getStyle("A11:L$line_item")->getFont()->setSize(8);
        //
        //Imprimir totales
        $line_item += 2;
        $sheet->setCellValue("K$line_item", 'TOTAL USD')->getStyle("K$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);;
        $sheet->setCellValue("L$line_item", 'TOTAL VES')->getStyle("L$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        $sheet->getStyle("A$line_item:L$line_item")->getFont()->setBold(true)->setSize(8);
        $line_item++;
        $sheet->mergeCells("H$line_item:J$line_item")->setCellValue("H$line_item", html_entity_decode('Sub-Total'));
        $sheet->getStyle("H$line_item:L$line_item")->getFont()->setBold(true)->setSize(8);
        $sheet->setCellValue("K$line_item", $sub_tot)->getStyle("K$line_item")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        $sheet->getCell("K$line_item")
            ->getStyle()->getNumberFormat()
            ->setFormatCode('#,###.00');
