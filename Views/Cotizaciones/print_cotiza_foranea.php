...
class PDF extends FPDF{
    ...
    function Header(){
        ...
        $this->Cell(20, 4,html_entity_decode($this->nom_tdo. ': ' . $this->num_tdo), 0,1); 
        $this->SetFont('Arial','',7);
        ...
        if($this->moneda_cot != $this->moneda_emp){
            $this->cell(40, 4, html_entity_decode('TOTAL USD'), 0, 0, 'R', );
        }
    ...
    function PrintTotal(){
        ...
        $this->cell(50);
        $this->Cell(20, 4, '$ ' . number_format(($GLOBALS['mon_base'] + $GLOBALS['mon_exe']) + ($GLOBALS['mon_base'] * ($GLOBALS['tasa_iva']/100)), 2, ",", "."), 0, 1, 'R');
    }
}
...
for ($i = 0; $i < count($r); $i++) {
    ...
    $nom_prod =$r[$i]->nom_prod . ' ' . $nom_fab . ' ' . $r[$i]->ref_prod;
    if($r[$i]->lote_prod){
...
$pdf->PrintChapter($GLOBALS['note_pre'] . PHP_EOL . ($GLOBALS['observa']));
$pdf->PrintTotal();
