               ...
               <tr title="<?= ($r->penapro == 1 ? "Pendiente por Aprobación" : "" )?>" <?= $style ?>>
                  ...
                  <td class="text-center">
                     <?php if (Permisos::updater() AND $r->status == "1") : ?>
                        <a type="button" class="btn btn-warning btn-xs" href="<?php echo base_url . '/Delnotnotfis/edit/' .  $r->id_cot; ?>" title="Editar"><i class="fa fa-edit"></i></a>
                     <?php endif ?>
                     ...
                     <?php endif ?>
                     <?php if(Permisos::deleter() AND $r->status == "1" ) : ?>
                        <a type="button" class="btn btn-success btn-xs" onclick="copiarBtn(this)" href="<?php echo base_url . '/Delnotnotfis/nueva/' .  $r->id_cot; ?>" title="Copiar"><i class="fa fa-copy"></i></a>
                     <?php endif ?>
                     <?php if(Permisos::updater() AND $r->status == "1" AND $r->penapro != 1) : ?> 
                        <button id="Data" data-id="<?php echo $r->id_cot ?>" data-name="<?php echo $r->nom_tdoc ?>" data-code = "<?php echo $r->num_tdo; ?>" type="button" class="btn btn-primary btn-xs" onclick="print_Delnotfis(this)" title="Imprimirr"><i class="fa-solid fa-print"></i></button>
                     <?php endif ?>
                  </td>
               </tr>
            <?php endforeach ?>
