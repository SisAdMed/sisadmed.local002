<?php
    ...
    class PDF extends FPDF{
        ...
        public $width, $height, $nom_tdoc, $tasa_cambio;
        //Totales
        ...
        public function __construct($r){
            ...
            $this->dir_emp = utf8_decode($r[0]->dir_emp);
            $this->nom_ent = utf8_decode($r[0]->nom_ent);
       ...
       /* $this->Image($this->ruta_logo,10, 8, 50, 0, 'PNG');
        ...
        $this->SetFont('Arial','B',7);
        $this->cell(30, 4, utf8_decode($this->nom_ent), 0);
        //Fecha
        $this->Cell(60);
        ...
        $this->cell(20, 4, 'RIF:');
        $this->SetFont('Arial','B',7);
        $this->cell(30, 4, utf8_decode($this->rif_ent));
        //Factura
        $this->Cell(60);
        $this->SetFont('Arial','',7);
        //$this->Cell(20, 4,utf8_decode($this->nom_tdoc), 0);
        $this->Cell(20, 4,utf8_decode('Número:'), 0);
        $this->SetFont('Arial','B',7);
        $this->Cell(20, 4,utf8_decode($this->num_tdo), 0);
        if($this->codigo_moneda != $this->moneda_emp){
        ...
        //Dirección
        $this->SetFont('Arial','',7);
        $this->cell(20, 4, utf8_decode('Dirección:'));
        $this->SetFont('Arial','B',7);
        $this->MultiCell(90, 4, utf8_decode($this->dir_ent . ' ' . strtoupper($this->nombre_ciudad) . ' ' . strtoupper($this->nombre_edo) . ' ' . strtoupper($this->nombre_pais)));
        $this->SetFont('Arial','',7);
        $this->cell(20, 4, 'Zona postal:');
        $this->SetFont('Arial','B',7);
        $this->cell(30, 4, utf8_decode($this->postal_ent));
        //Titulos
        $this->ln();
        $this->cell(100, 4, utf8_decode('DESCRIPCIÓN'), 0, 0, 'C');
        $this->Cell(20);
        $this->cell(5, 4, utf8_decode('IVA'), 0, 0, 'C');
        $this->cell(15, 4, utf8_decode('CANT.'), 0, 0, 'C');
        $this->cell(10, 4, utf8_decode('PRECIO'), 0, 0, 'R');
        if($this->moneda_cot != $this->moneda_emp){
            $this->cell(20, 4, utf8_decode('TOTAL USD'), 0, 0, 'R', );
            $this->cell(20, 4, utf8_decode('TOTAL VEB'), 0, 0, 'R', );
        }else{
            $this->cell(40, 4, utf8_decode('TOTAL VEB'), 0, 0, 'R', );
        }
        $this->ln(5);
         // Guardar ordenada
        $this->y0 = $this->GetY();
    ...
    function ChapterBody($file){
        ...
        $this->MultiCell(100,3,utf8_decode($txt), 0, 'C');
        $this->Ln();
    ...
    function PrintTotal(){
        ...
        $this->cell(100);
        $this->SetFont('Arial','B', 7);
        $this->cell(20, 4, utf8_decode('Sub-Total'), 0, 0, 'L');
        if($GLOBALS['moneda_cot'] != $GLOBALS['moneda_emp']){
            ...
            //Exento
            $this->cell(100);
            $this->cell(20, 4, utf8_decode('Sub-Total Exento'), 0, 0, 'L');
            $this->cell(30);
            ...
            //Base imponible
            $this->cell(100);
            $this->cell(20, 4, utf8_decode('Sub-Total Base Imponible'), 0, 0, 'L');
            $this->cell(30);
            ...
            //IVA
            $this->cell(100);
            $this->cell(20, 4, utf8_decode('IVA 16.00% sobre ') . number_format($GLOBALS['mon_base'],2, ",", "."), 0, 0, 'L');
            $this->cell(30);
            ...
            //Total
            $this->cell(100);
            $this->cell(20, 4, utf8_decode('Total'), 0, 0, 'L');
            $this->cell(30);
...
for ($i = 0; $i < count($r); $i++) {
    ...
    $pdf->MultiCell(120, 3, utf8_decode($nom_prod), 0, 'L');
    $pdf->ln(-3);
    $pdf->Cell(120);
    $pdf->Cell(5, 3, utf8_decode($r[$i]->iva_prod ? '16.00' : '(E)'), 0, 0, 'C');
...
$pdf->PrintChapter($GLOBALS['note_fac']);
$pdf->PrintTotal();

$pdf->Output('', utf8_decode($filemame));
?>