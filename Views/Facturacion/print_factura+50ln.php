<?php
    ...
    class PDF extends FPDF{
        ...
        if($this->moneda_cot != $this->moneda_emp){
            $this->Cell(10);
            $this->cell(10, 4, html_entity_decode('IVA'), 0, 0, 'C');
    ...
    function PrintTotal(){
        ...
        //$this->SetY(-15);
        $increLn = 5;
        $this->cell(100+ $increLn);
        $this->SetFont('Arial','B', 7);
        $this->cell(20 + $increLn, 4, html_entity_decode('Sub-Total'), 0, 0, 'L');
        if($GLOBALS['moneda_cot'] != $GLOBALS['moneda_emp']){
            //Sub-total
            $this->cell(30 - $increLn);
...
for ($i = 0; $i < count($r); $i++) {
    if($r[$i]->print_lote == 1 && $r[$i]->lote_prod == 1){
        ...
        $nom_prod = html_entity_decode($r[$i]->nom_prod) . ' Marca ' . html_entity_decode($r[$i]->nom_fab) . ' Ref. ' . html_entity_decode($r[$i]->ref_prod) . ' Lote: ' . $num_lote . ' Fec.Venc. ' . $fec_venc;
    ...
    if($r[$i]->codigo_moneda != $r[$i]->moneda_emp){
        $cadena = iconv("UTF-8", "ISO-8859-1", $nom_prod);
        $pdf->MultiCell(95, 3, html_entity_decode($cadena), 0, 'L');
        $pdf->ln(-3);
        $pdf->Cell(95);
        if($r[$i]->iva_prod == 'S'){
