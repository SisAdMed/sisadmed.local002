            ...
            <div class="col-sm-12">
               ...
               <?php if (Permisos::create()) : ?>
                  <ol class="breadcrumb float-sm-right">
                     <li class="breadcrumb-item"><a href="<?= base_url ?>/FacNotCre/nuevo" title="Nuevo registro"><i class="fa fa-plus-circle"></i></a></li>
                  </ol>
               <?php endif ?>
            </h1>
         </div>
               ...
               <tr title="<?= ($r->penapro == 1 ? "Pendiente por Aprobación" : "" )?>" <?= $style ?>>
                  ...
                  <td class="text-center">
                     <?php if (Permisos::updater() AND $r->status == "1") : ?>
                        <a type="button" class="btn btn-warning btn-xs" href="<?php echo base_url . '/FacNotCre/edit/' .  $r->id_cot; ?>" title="Editar"><i class="fa fa-edit"></i></a>
                     <?php endif ?>
                     ...
                     <?php endif ?>
                     <?php if(Permisos::deleter() AND $r->status == "1" ) : ?>
                        <a type="button" class="btn btn-success btn-xs" onclick="copiarBtn(this)" href="<?php echo base_url . '/FacNotCre/nueva/' .  $r->id_cot; ?>" title="Copiar"><i class="fa fa-copy"></i></a>
                     <?php endif ?>
                     <?php if(Permisos::updater() AND $r->status == "1" AND $r->penapro != 1) : ?>
                        <a type="button" class="btn btn-primary btn-xs" data-code = "<?php echo $r->num_tdo; ?>" href="<?php echo base_url . '/FacNotCre/print_factura/' .$r->id_cot ?>" target="_blank"><i class="fa-solid fa-print" title="Imprimir"></i></a>
                     <?php endif ?>
                  </td>
               </tr>
            <?php endforeach ?>
