...
class DelnotModel extends DB{
    ...
    static function print_factura($id_cot){
        $r = DB::query("SELECT em.id_emp, em.nombre_emp nombre_emp, em.rif_empresa rif_empresa, em.dir_emp dir_emp, em.tel_emp tel_emp, em.email_emp email_emp, c.num_tdo num_tdo, tdo.nom_tdoc nom_tdoc, c.fecha_comp fecha_comp, en.nom_ent nom_ent, en.rif_ent rif_ent, en.dir_ent dir_ent, en.postal_ent postal_ent, pa.nombre_pais nombre_pais, es.nombre_edo nombre_edo, ci.nombre_ciudad nombre_ciudad, ve.nom_vend nom_vend, ve.ape_vend ape_vend, pro.cod_prod cod_prod, pro.cod2_prod cod2_prod, pro.nom_prod nom_prod, dc.iva_prod iva_prod, dc.can_det can_det, dc.pre_vta pre_vta, dc.sub_total sub_total, c.id_cot id_cot, em.logo logo, moc.codigo_moneda codigo_moneda, moe.codigo_moneda moneda_emp, c.tasa_cambio tasa_cambio, cfg.id_tdoc_fac, cfg.note_fac, cfg.id_tdoc_cre, cfg.note_cre, cfg.id_tdoc_pre, cfg.note_pre, cfg.id_tdoc_not, cfg.note_not, cfg.id_tdoc_dev, cfg.note_dev, en.note_fac note_fac_custom, fab.nom_fab, pro.ref_prod, c.oc_cliente, c.descrip_cot, en.print_lote, null nom_fab_fac FROM f6003 c INNER JOIN f6001 tdo ON tdo.id_tdoc = c.id_tdo INNER JOIN f60031 dc ON dc.id_cot = c.id_cot INNER JOIN f0011 em ON em.id_emp = c.id_emp INNER JOIN f0014 en ON en.id_ent = c.id_cli INNER JOIN f0004 pa ON pa.id_pais = en.id_pais INNER JOIN f00041 es ON es.id_edo = en.id_edo INNER JOIN f00042 ci ON ci.id_ciudad = en.id_ciudad INNER JOIN f0016 ve ON ve.id_vend = en.id_vend INNER JOIN f4005 pro ON pro.id_prod = dc.id_prod INNER JOIN f0005 moc ON moc.id_moneda = c.id_moneda INNER JOIN f0005 moe ON moe.id_moneda = em.id_moneda INNER JOIN f4003 fab ON fab.id_fab = pro.id_fab INNER JOIN f4999 cfg ON cfg.id_emp = c.id_emp WHERE c.id_cot = {$id_cot}");
        return $r;
    }
    ...
    static function borrarDetInvMov($id){
        return $r = DB::delete('f40091', ['id_movinv' => $id]);
    }
    ...
    static function consulta_prod_ent_alm($id, $id_alm, $id_ent){
        $r = DB::query("SELECT * FROM f4010 WHERE id_prod = {$id} AND id_alm = {$id_alm} and id_ent = {$id_ent} AND stock > 0");
        return $r;
    }
    ...
    static function getNumberMovim($origen, $id_tmovinv){
        $r = DB::query("SELECT * FROM f4009 WHERE origen = '$origen' AND id_tmovinv = {$id_tmovinv}");
        return $r[0];
    }
    ...
    static function consulta_vend($id_ent){
        $r = DB::query("SELECT DISTINCT a.id_vend, a.id_moneda, case when ISNULL(b.cod_diascre) THEN 0 ELSE b.cod_diascre END cod_diascre, a.nom_ent, a.c_consig, a.id_alm, a.id_ubi FROM f0014 a LEFT OUTER JOIN f6005 b on b.id_diascre = a.id_diascre WHERE a.id_ent = {$id_ent}");
        return $r[0];
    }
