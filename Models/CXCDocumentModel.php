...
class CXCDocumentModel extends DB
{
    ...
    static function edo_cuenta($id_emp, $id_cli, $ori)
    {
        ...
        if ($ori == 0) {
            $sql = "SELECT nombre_emp, nom_ent, nom_tdoc, fecha_comp, cod_diascre, dias_calle, num_tdo, tasa_cambio, adic_01 motivo, status_doc, mon_exe_dom, mon_base_dom, mon_iva_dom, ret_iva_dom, dif_iva_dom, por_cob_dom, mon_exe_for, mon_base_for, mon_iva_for, ret_iva_for, dif_iva_for, por_cob_for FROM view_doc_pend_cxc WHERE id_emp = {$id_emp} $filter";
        }else{
    ...
    static function ventas_globales()
   {
    ...
    static function show_row($id){
        $sql = "SELECT a.id_cot, a.id_emp, a.id_tdo, a.num_tdo, a.nro_control, a.id_cli, a.fecha_comp, a.fecha_venci, a.id_moneda, a.tasa_cambio, a.descrip_cot, a.id_doc_afec, a.doc_afectado, a.status, d.id_concxc, CONCAT(n.codigo_con, ' - ', n.nombre_con) nombre_con, o.id_aux, CONCAT(o.cod_aux, ' - ', o.nombre_aux) nombre_aux , d.monto, CASE WHEN d.mon_iva <> 0 THEN 'S' ELSE 'N' END iva, d.mon_iva FROM f6003 a INNER JOIN f0011 b ON b.id_emp = a.id_emp INNER JOIN f6001 c ON c.id_tdoc = a.id_tdo INNER JOIN f60032 d ON d.id_cot = a.id_cot INNER JOIN f0014 e ON e.id_ent = a.id_cli INNER JOIN f0005 f on f.id_moneda = a.id_moneda LEFT OUTER JOIN f60031 l ON l.id_cot = a.id_cot LEFT OUTER JOIN f6003 m ON m.id_cot = a.id_doc_afec INNER JOIN f6002 n ON n.id = d.id_concxc LEFT OUTER JOIN f0009 o ON o.id_aux = d.id_aux WHERE a.id_cot = {$id}";
        $r = DB::query($sql);
        return $r;
    }
    /**
